...
def similarity(input_file,auth_key):
    ...
    try:
        ...
        if os.path.exists(input_file):
            ...
            sysdate = strftime("%Y-%m-%d_%H_%M_%S", gmtime())
            ...
            for file in doc_labels:
                with open(file, 'r', errors='ignore', encoding='utf-8') as f:
                    training_dataset.append(((os.path.basename(os.path.dirname(file)) + '~~__~~' + f.read()).replace("\n", ' ')) + "\n")
...
if __name__ == '__main__':
     print("\n MAIN:")
     #start_time = time()
     print(similarity('\\\\192.168.0.14\\uat\\unclassified_doc\\template\\1\\1~~__~~6_passport_image.txt'))
