...
def call_classiciation():
    ...
    try:
        ...
        logger.log('sec_token  -- >',sec_token)
                ...
                if resp and resp.status_code == 200 and discovery_file_details is not None and len(discovery_file_details) > 0:
                    ...
                    for document in discovery_file_details:
                        try:
                            ...
                            payload['IsTrainingSource'] = 0
                            if document['SourceTag'] == 'CRM' and json_str.get("FileFullPath").strip() != '' and json_str.get("Annotationid").strip() != '' :
                                crm_document_reqst = req.request(method='GET', url=json_str.get("FileFullPath"))
                                crm_document_data = crm_document_reqst.json()
                                logger.log("crm_document_data[0]", crm_document_data[0])
                                if crm_document_reqst and crm_document_reqst.status_code == 200 and crm_document_data is not None and os.path.exists(
                                crm_document_data[0]):
                                    payload['file_name'] =  crm_document_data[0]
                                else:
